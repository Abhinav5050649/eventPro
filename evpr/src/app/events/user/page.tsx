  ...
  return (
    <>
      ...
      {filteredData.map((event: Event) => (
        <div
          key={event._id}
          className="max-w-sm text-white bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"
        >
          <a href={`/events/view/:${event._id}`}>
            <img
              className="rounded-t-lg"
              src={event.image || process.env.DEFAULT_EVENT_IMG}
              alt="Event Image"
            />
          </a>
          <div className="p-5">
