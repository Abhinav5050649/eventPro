...
export default function EventUpdateForm({ params }: any) {
    ...
    const fetchData = async () => {
      try {
        ...
        setData({
          ...
          miscLinks: eventData.miscLinks,
        });

        setStDate(new Date(eventData.startDate).toISOString().split("T")[0]);
        setEnDate(new Date(eventData.endDate).toISOString().split("T")[0]);
      } catch (error: any) {
  ...
  const handleRegistration = async () => {
    try {
      ...
      const response = await axios.put(`/api/events/update/${id}`, updatedData);
                  ...
                  <div className="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                    <div className="flex flex-col">
                      ...
                      <input
                        ...
                        placeholder="Enter updated name/title"
                        value={data.name}
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="relative focus-within:text-gray-600 text-gray-400">
                          <input
                            ...
                            placeholder={`Format: dd/mm/yyyy --> Date now: ${stDate}`}
                            value={stDate}
                            onChange={(e) => setStDate(e.currentTarget.value)}
                            required
                          />
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="relative focus-within:text-gray-600 text-gray-400">
                          <input
                            type="text"
                            className="pr-4 pl-10 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                            placeholder={`Format: dd/mm/yyyy --> Date now: ${enDate}`}
                            value={enDate}
                            onChange={(e) => setEnDate(e.currentTarget.value)}
                            required
                          />
                          <div className="absolute left-3 top-2">
                            <svg
                    ...
                    <div className="flex flex-col">
                      ...
                      <input
                        type="text"
                        className="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                        placeholder="Optional. If multiple, separate by ;"
                        value={data.miscLinks}
                        onChange={(e) => setData({ ...data, miscLinks: e.target.value })}
                      />
                    </div>
                  </div>
